...
bool minconTMINLP::getHessFromScilab(Index n, wchar_t* name, const Number* x, double obj, const Number* lambda, double * resCh)
{
    double check;
...
bool minconTMINLP::eval_h(Index n, const Number* x, bool new_x,Number obj_factor, Index m, const Number* lambda,bool new_lambda, Index nele_hess, Index* iRow,Index* jCol, Number* values)
{
    ...
    else
    {
        ...
        if (getHessFromScilab(n, L"_gradhess", x, obj_factor, lambda, resCh) !=1)
        {
            sciprint("Hessian error");
        }
